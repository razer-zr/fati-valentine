...
export default function Ask(props) {
  ...
  const [askText, setAskText] = useState("Will you do me the honor of being my one and only Valentine :)");
  ...
  const handleNoClick = (event) => {
    ...
    if (noCount === 0) {
      ...
      setAskText("Why tf would you say No you WANKER?")
      setEmojiAsk('😢')
    } else if (noCount === 1) {
      ...
      setAskText("You got no choice but to be my Valentine...... tf you think this is ???")
      ...
      setAskText("Bye You don't deserve me. Tell Ammu I'll miss her")
      setEmojiAsk('');
  ...
  const handleYesClick = (event) => {
    ...
    setAskText("Lets goooooooooooo !!! I'm offically the happiest man alive with the best girlfriend")
    setEmojiAsk('😍');
    setYesOrNow("Yes");
  }
  ...
  return (
    <div className='message-sending-body flex items-center flex-col w-full h-full bg-transparent bg-gradient-to-tr from-[#F56217] to-[#0B486B] min-h-screen gap-6 sm:gap-20'>
      ...
      <div className="ask-convey px-4 flex flex-col flex-wrap items-center gap-3 sm:gap-7">
        <div className='convey h-fit w-fit text-transparent bg-clip-text  bg-gradient-to-t from-[#962820] to-[#111125] sm:text-4xl text-center select-none'>In a world  with over 4 billion boring people (maybe?), you stand out to me as THE MOST beautiful and charming girlfriend EVER.</div>
